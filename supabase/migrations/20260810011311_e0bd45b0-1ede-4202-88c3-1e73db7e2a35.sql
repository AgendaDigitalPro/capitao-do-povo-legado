...
BEGIN
  ...
  INSERT INTO public.pedidos (
    ...
    p_whatsapp,
    p_email,
    COALESCE(p_status, 'aguardando_pagamento')
  )
...
REVOKE ALL ON FUNCTION public.salvar_etapa_pedido(text, text, text, text, text, text, text, text) FROM PUBLIC;
GRANT EXECUTE ON FUNCTION public.salvar_etapa_pedido(text, text, text, text, text, text, text, text) TO anon, authenticated, service_role;